...
function Dc() {
  ...
  return (
    ...
    <div  className='flex flex-wrap pt-2 justify-center '>
      ...
      {data && data.map((datas) => (
        <div id='card' className='rounded-lg shadow-2xl bg-white m-4 p-4 ' key={datas.id}>
      ...
      {data1 && data1.map((datas1) => (
        <div id='card' key={datas1.id} className='rounded-lg shadow-2xl bg-white m-4 p-4 '>
    ...
    <div className='rounded-lg shadow-2xl bg-white m-4 p-40 '>
        <button onClick={()=>navigate("/addDcMovies")} className='bg-blue-600 text-2xl p-8 rounded-sm text-white'>+</button>
   </div>
    </div>
    </>
  );
